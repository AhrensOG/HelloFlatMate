                ...
                <main className={`flex flex-col gap-[2.5rem] grow text-[#0D171C] w-full px-3`}>
                    <div className="w-full space-y-2 sticky top-0 min-h-56 bg-white z-10 pt-2 pb-1">
                        ...
                        <h4 className="text-base font-bold text-resolution-blue">
                            {isLeaseOrderActive ? `Habitacion libre a partir de ${formatDateToDDMMYYYY(isLeaseOrderActive.endDate)}` : ""}
                        </h4>
                        {roomData.price && <PriceSection data={roomData.price} />}
                        {(data.category.name === "HELLO_ROOM" ||
                            data.category.name === "HELLO_COLIVING" ||
                            data.category.name === "HELLO_LANDLORD") &&
                            roomData.price && <ReservationButton callback={handleShowModal} disabled={isLeaseOrderActive || false} />}
                    </div>
                ...
                <main className={`  flex flex-row gap-10 grow p-5 text-[#0D171C] w-full max-w-screen-2xl px-3`}>
                    ...
                    <div className="w-full space-y-4">
                        ...
                        {filteredRooms.length > 0 ? (
                            <RoomSection data={filteredRooms} title="Otras habitaciones en el mismo piso" category={data.category.name} />
                        ) : null}
                    </div>
