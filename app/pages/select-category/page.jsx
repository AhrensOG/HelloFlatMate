...
const SelectCategoryPage = () => {
    ...
    useEffect(() => {
        if (state.properties && state.properties.length > 0) {
            ...
            const roomProps = state.properties.filter((property) => property.category.name === "HELLO_ROOM");
            const colivingProps = state.properties.filter((property) => property.category.name === "HELLO_COLIVING");
            const studioProps = state.properties.filter((property) => property.category.name === "HELLO_STUDIO");
            const landlordProps = state.properties.filter((property) => property.category.name === "HELLO_LANDLORD");
...
const Loader = () => {
    return (
        <div className="w-full h-screen flex justify-center items-center">
            <div className="loader"></div> {/* Puedes crear una animación CSS para el loader */}
            <style jsx>{`
                .loader {
