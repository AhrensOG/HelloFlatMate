...
export async function deleteRentalPeriodById(id) {
    ...
    try {
        ...
        try {
            ...
            return NextResponse.json({ message: "Error deleting rental period" }, { status: 500 })
        }
    } catch (error) {
        console.log(error)
        return NextResponse.json({ message: "Error deleting rental period" }, { status: 500 })
...
export async function deleteGroupRentalPeriods(ids) {
    ...
    try {
        ...
        try {
            ...
            if (!rentalPeriods) {
                return NextResponse.json({ message: "Rental periods not found" }, { status: 404 })
            }
            ...
            transaction.rollback();
            return NextResponse.json({ message: "Error deleting rental periods" }, { status: 500 })
        }
    } catch (error) {
        return NextResponse.json({ message: "Error deleting rental periods" }, { status: 500 })
    }
