...
export async function manualUpdateSupply(data) {
    if (!data.id) return NextResponse.json({ error: "No supply ID provided" }, { status: 400 });
    ...
    try {
        ...
        await supply.update({
            ...
            paymentDate:
                data.paymentDate && data.paymentDate !== "" ? new Date(data.paymentDate) : data.paymentDate === "" ? null : supply.paymentDate,
            paymentId: data.paymentId ? data.paymentId : data.paymentId === "" ? null : supply.paymentId,
