...
export async function fullUpdateLeaseOrder(data) {
    if (!data.leaseOrderId) {
        return NextResponse.json({ message: "No lease order ID provided" }, { status: 400 });
    }

    if (!data.type || (data.type !== "property" && data.type !== "room")) {
        return NextResponse.json({ message: "Invalid or no type provided. Must be 'property' or 'room'" }, { status: 400 });
    }
    ...
    try {
        ...
        if (!leaseOrder) {
            ...
            return NextResponse.json({ message: "Lease order not found" }, { status: 404 });
        }
        ...
        return NextResponse.json({ message: "Lease order updated successfully", leaseOrder }, { status: 200 });
    } catch (error) {
