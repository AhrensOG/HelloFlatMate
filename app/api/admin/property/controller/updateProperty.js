...
export async function updateProperty(id, data) {
    ...
    try {
        const property = await Property.findByPk(id, {
            include: [
                { model: Chat, as: "chats" },
                { model: Room, as: "rooms", include: { model: Chat, as: "chats" } },
            ],
        });
        if (!property) {
        ...
        if (data.deleteRentalPeriods?.length > 0) {
            await Promise.all(
                data.deleteRentalPeriods?.map(async (period) => {
    ...
    try {
        ...
        try {
            ...
            for (const category of data.categories) {
                ...
                await Promise.all(properties.map((property) => property.update({ description: accData.descriptions, ...accData }, { transaction })));
            }
...
export async function updateSimpleProperty(data) {
    try {
        if (!data) {
            return NextResponse.json({ error: "Need data" }, { status: 400 });
        }
