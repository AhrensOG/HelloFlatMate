...
export async function GET(req) {
  try {
    ...
    const leaseOrders = await LeaseOrderRoom.findAll({
      where: { isActive: true, endDate: { [Op.lte]: new Date() } },
    });

    if (leaseOrders.length === 0) {
      return NextResponse.json(
        { message: "No lease orders to update" },
        { status: 200 }
      );
    }

    await LeaseOrderRoom.update(
      { isActive: false, status: "FINISHED" },
      {
        where: {
