    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-[#A1A0A066] z-50 backdrop-blur-sm">
            <aside className="bg-white p-6 rounded-lg w-72 z-50">
                <h2 className="font-semibold mb-4 text-center text-base">{type === "problem" ? t("h2_1") : t("h2_2")}</h2>
                <form action="">
                    <label className={`${type === "problem" ? "hidden" : ""} block text-sm font-medium text-gray-700 text-center`} htmlFor="report">
                        {t("title")}
                    </label>
                    <textarea
