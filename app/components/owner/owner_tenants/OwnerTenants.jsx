...
const OwnerTenants = ({ data = false }) => {
    ...
    const t = useTranslations("owner_panel.owner_tenants");
                        ...
                        <div className="w-full max-w-screen-md flex flex-grow flex-col justify-start items-center gap-4">
                            {data &&
                                data.map((tenant) => {
                                    ...
                                    const image = tenant.profilePicture === "" || !tenant.profilePicture ? false : tenant.profilePicture;
                ...
                {isModalOpen && (
                    ...
                    >
                        <div className="bg-white absolute top-0 rounded-lg w-full flex justify-center items-center">
                            <OwnerTenantDetail tenant={selectedTenant} action={handleCloseModal} />
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
