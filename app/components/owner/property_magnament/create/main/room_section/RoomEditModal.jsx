...
export default function RoomEditModal({ data, setData, showModal }) {
  const placeholder = "URL de la imagen";

  const [items, setItems] = useState(data);

  useEffect(() => {
    // Asegúrate de que haya al menos una URL al iniciar
    if (items.length < 1) {
      setItems([...items, ""]);
    }
  }, []);
  ...
  const deleteItem = () => {
    if (items.length > 1) {
      // Permite eliminar si hay más de una URL
  ...
  const saveData = () => {
    ...
    if (filteredItems.length < 1) {
      return toast.info("Por favor, agrega al menos una URL válida.");
    }
