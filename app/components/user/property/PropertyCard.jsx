...
export default function PropertyCard({ name, images, property, price, roomId = false, room = false }) {
    const [isOpen, setIsOpen] = useState(false);
    const route = useRouter();

    const handleRedirect = () => {
        if (
            (property.category.name === "HELLO_ROOM" || property.category.name === "HELLO_COLIVING" || property.category.name === "HELLO_LANDLORD") &&
            roomId
        ) {
            route.push(`/pages/property-details/${property.id}/room-details/${roomId}`);
        } else {
            ...
            <div onClick={handleRedirect} className="flex sm:flex-col gap-3 sm:gap-0 w-full h-full">
                <div className="relative h-28 w-28 sm:w-72 sm:h-80 rounded-md">
                    <Image
                        ...
                        src={images || property?.property?.images[0] || property?.images[0] || ""}
                        fill
                    ...
                    <div className="flex flex-col grow sm:gap-2">
                        <h4 className="flex w-full gap-2 items-center text-xs text-[#000000B2] font-normal">
                            ...
                            property?.category.name === "HELLO_LANDLORD"
                                ? "HABTITACION"
                                : property?.category.name.toLowerCase().split("_").join("")}
                            {/* <button
                type="button"
                className="relative"
                            ...
                            <span className="text-xs font-medium text-black/80">
                                {room?.leaseOrdersRoom?.some((order) => order.isActive === true)
                                    ? // Si hay una leaseOrder activa, mostrar la fecha de disponibilidad
                                      `Disponible ${formatDate(room?.leaseOrdersRoom.find((order) => order.isActive === true)?.endDate)}`
                        ...
                        <p className="text-[0.68rem] text-[#828282] font-normal">
                            {
                                ...
                                Array.isArray(property?.property?.amenities) && property?.property?.amenities?.length > 0
                                    ? property?.property?.amenities
                                          ...
                                          .join(", ") + (property?.property?.amenities.length > 3 ? ", ..." : "")
                                    : Array.isArray(property?.amenities) && property?.amenities?.length > 0
                                    ? property?.amenities
                                          ?.slice(0, 3) // Obtener solo los primeros 3 elementos
                                          .map((amenity) => amenity.charAt(0).toUpperCase() + amenity.slice(1))
                                          .join(", ") + (property?.amenities.length > 3 ? ", ..." : "")
                                    : ""
                            }
                        </p>
                    </div>
