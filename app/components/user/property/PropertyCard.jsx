...
export default function PropertyCard({ name, images = "/not-image.png", property, price, roomId = false, room = false }) {
    ...
    return (
        <article className={`  flex flex-col max-h-96 h-full gap-3 w-full sm:max-w-72 cursor-pointer border sm:border-none rounded-sm`}>
                                ...
                                {room?.leaseOrdersRoom?.some((order) => order.isActive === true)
                                    ? // Si hay una leaseOrder activa, mostrar la fecha de disponibilidad
                                      `Disponible ${formatDate(
                                          (() => {
                                              const activeOrder = room?.leaseOrdersRoom.find((order) => order.isActive === true);
