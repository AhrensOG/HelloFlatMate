...
export default function AmenitiesSection({ edit = null, data = null }) {
    const t = useTranslations("room_details.amenities");
        ...
        <section className="flex flex-col gap-3">
            {edit ? (
                <div className="w-full flex justify-between items-center">
                    <h2 className="font-bold text-[1.2rem]">{t("h2")}</h2>
                    {edit}
                </div>
            ) : (
                <h2 className="font-bold text-[1.2rem]">{t("h2")}</h2>
            ...
            {data ? (
                data.map((item) => <Amenity key={item} name={item} icon={handleIcon(item)} />)
            ) : (
                <>
                    <Amenity name="Piscina disponible" icon={handleIcon("jardin")} />
                    <Amenity name="Wifi" icon={handleIcon("wifi")} />
                    <Amenity name="Estacionamiento" icon={handleIcon("estacionamiento")} />
                </>
            )}
        </section>
    );
