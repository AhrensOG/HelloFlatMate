...
export default function MyBedroomDetails({ room }) {
    ...
    const handlePayment = async (price, quota) => {
        console.log(price, quota);

        const data = {
            amount: price,
            type: "MONTHLY",
            paymentableId: room?.id,
            paymentableType: room?.type === "HELLO_ROOM" || room?.type === "HELLO_COLIVING" ? "ROOM" : "PROPERTY",
            clientId: room?.leaseOrder?.clientId,
            leaseOrderId: room?.leaseOrder?.id,
            leaseOrderType: room?.type === "HELLO_ROOM" || room?.type === "HELLO_COLIVING" ? "ROOM" : "PROPERTY",
            quotaNumber: quota,
        };

        try {
            const res = await axios.post("/api/rent_payment", data);
            ...
            <section className="w-full flex flex-col gap-5">
                ...
                <div className="text-sm flex flex-col gap-2">
                    <div className="flex justify-between">
                        <p className="font-light">{type === "HELLO_STUDIO" ? "Finalización" : "Vencimiento"}</p>
