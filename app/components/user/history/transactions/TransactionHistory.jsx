...
export default function TransactionHistory({ redirect }) {
    ...
    try {
      ...
      if (missingFields.length > 0) {
        toast.info(
          ...
          { description: `¡Dirigete a 'Perfil' para hacerlo!`, id: toastId }
        );
        return;
      }
  ...
  const renderRentPayments = () => {
    ...
    const getMonthFromLeaseOrder = (startDate, quotaNumber) => {
      ...
      date.setMonth(date.getMonth() + quotaNumber - 1);
          ...
          <div className="grid gap-1">
            {rentPayments.length > 0 ? (
              rentPayments.map((payment) => {
                const monthLabel = getMonthFromLeaseOrder(
                  payment.startDate,
                  payment.quotaNumber
                );
              ...
              <p className="text-gray-500">
                No hay pagos mensuales disponibles.
              </p>
            )}
          </div>
        </motion.div>
