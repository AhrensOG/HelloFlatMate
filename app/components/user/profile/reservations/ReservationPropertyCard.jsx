...
export default function ReservationPropertyCard({ property, leaseOrder }) {
    ...
    const [category, setCategory] = useState(property?.property?.category.name || property?.category.name);

    const handleRedirect = () => {
        if (category === "HELLO_ROOM" || category === "HELLO_COLIVING" || category === "HELLO_LANDLORD") {
            route.push(`/pages/property-details/${property.propertyId}/room-details/${property.id}`);
        } else {
    ...
    const handleRedirectToContract = () => {
        if (category === "HELLO_ROOM" || category === "HELLO_COLIVING" || category === "HELLO_LANDLORD") {
            route.push("/pages/user/contract/sign-contract/" + property?.propertyId + "?r=" + property.id + "&lo=" + leaseOrder.id);
        } else {
            route.push("/pages/user/contract/sign-contract/" + property?.id + "?lo=" + leaseOrder.id);
        }
    };
    ...
    return (
        <article
            className={`  max-w-[350px] sm:max-w-60 sm:h-[310px] flex flex-col gap-3 w-full cursor-pointer border p-2 rounded-xl shadow-reservation-drop`}
        >
            <div onClick={handleRedirect} className="flex sm:flex-col gap-3 w-full sm:grow">
                <div className="relative h-28 w-28 sm:w-[222px] rounded-xl">
                    <Image className="h-full rounded-xl" src={property?.images[0] || ""} fill alt="Imagen de propiedad" />
                </div>
                <div className="flex flex-col justify-between grow gap-2">
                    <div className="flex flex-col grow gap-2">
                        <div className="flex w-full gap-2 justify-between items-center text-[0.81rem] text-[#000000B2] font-normal">
                            ...
                            <div>
                                <h3 className="text-xs text-[#000000B2]">{formatDate(leaseOrder.date)}</h3>
                            </div>
                        </div>
              ...
              {category === "HELLO_ROOM" || category === "HELLO_COLIVING"
                ...
                : property?.amenities
                    ...
                    <div className="flex flex-1 justify-between items-center gap-2">
                        <div>
                            <h3 className="text-xs text-[#000000B2]">Ingreso: {formatDate(leaseOrder.startDate)}</h3>
                            <h3 className="text-xs text-[#000000B2]">Salida: {formatDate(leaseOrder.endDate)}</h3>
                        </div>
                        <div className="flex flex-col justify-end items-end font-medium">
                            {property?.price > 0 && (
                                <h3 className="text-base text-[#000000B2]">
                                    {property?.price} <span className="text-xs text-[#B2B2B2]">/mes</span>
                                </h3>
                            )}
                        </div>
                    </div>
            ...
            {leaseOrder.isSigned && leaseOrder.status === "APPROVED" ? (
                ...
                >
                    Contrato Firmado
                    <QuestionMarkCircleIcon title="¡Tu solicitud fue aprobada y el contrato firmado!" className="size-4" />
                </p>
            ) : !leaseOrder.isSigned && leaseOrder.status === "APPROVED" ? (
            ...
            ) : !leaseOrder.isSigned && leaseOrder.status === "PENDING" && leaseOrder.inReview ? (
                ...
                <div className="w-full flex flex-col justify-center items-center gap-1">
                    ...
                    >
                        En revisión
                        <QuestionMarkCircleIcon title="¡Tu solicitud esta bajo revisión. Te notificaremos al finalizar!" className="size-4" />
                    </p>
                    <button
