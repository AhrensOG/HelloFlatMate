...
export default function RentalPeriodsPanel() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await mutate();
      toast.success("Periodo de renta eliminado correctamente", {
        id: toastId,
      });
    } catch (error) {
      console.error("Error deleting item:", error);
      toast.error("Ocurrio un error al eliminar el periodo de renta", {
            ...
            {swrData?.rentalPeriods &&
              swrData?.rentalPeriods?.map((period) => (
                <tr
                  ...
                  <td className="border p-2 text-gray-700 text-center">
                    {formatDateToDDMMYYYY(period.startDate)}
                  </td>
                  <td className="border p-2 text-gray-700 text-center">
                    {formatDateToDDMMYYYY(period.endDate)}
                  </td>
                  <td className="border p-2 text-gray-700 text-center">
                    <div className="flex items-center justify-around">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toast("Eliminar periodo de alquiler", {
                            action: {
                              label: "Confirmar",
