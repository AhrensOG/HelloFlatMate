...
export default function RoomsPanel() {
  ...
  const filteredData = swrData
    ?.filter((room) => {
      ...
      const matchesSearch = roomString.includes(query);
  ...
  const handleOnsave = async (data) => {
    ...
    try {
      await axios.put(
        `/api/admin/room/new_panel?id=${selectedRoom.id}`,
        roomData
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.info("Ocurrio un error al eliminar la habitación", {
          ...
          <tbody>
            {filteredData &&
              filteredData.map((room) => {
                ...
                const order =
                  room.leaseOrdersRoom?.length > 0 &&
                  room.leaseOrdersRoom?.find((order) => order.isActive);
