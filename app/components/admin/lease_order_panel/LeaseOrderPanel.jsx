...
export default function LeaseOrderPanel({ data }) {
    ...
    const handleLeaseOrderAction = async (leaseOrder, action) => {
        const dataRequest = {
            ...
            propertyId: null,
            type: "ROOM",
            roomId: leaseOrder.roomId,
        };

        try {
            toast.promise(axios.patch(`/api/admin/lease_order`, dataRequest), {
                loading: "Procesando...",
                success: `Orden ${action === "PENDING" ? "aprobada" : "rechazada"} exitosamente`,
    ...
    const renderLeaseOrders = (orders, roomdata, isCurrent) =>
        orders.map((leaseOrder) => (
            <div key={leaseOrder.id} className="my-4">
                <LeaseOrderSection data={leaseOrder} formatDate={formatDate} room={roomdata} />
                <LeaseOrderClientSection data={leaseOrder.client} formatDate={formatDate} isSigned={leaseOrder.isSigned} />
        ...
        <main className="max-w-4xl mx-auto my-8 p-6 bg-white rounded-lg shadow-lg">
            ...
            <div className="flex flex-col gap-2">
                ...
                >
                    ...
                    <ChevronUpIcon className={`w-6 h-6 transform transition-transform ${showCurrentLeaseOrder ? "rotate-180" : ""}`} />
                </div>
                <AnimatePresence>
                                      ...
                                      <div key={room.id}>
                                          {renderLeaseOrders(
                                              room.leaseOrdersRoom.filter(
                                                  (order) =>
                                                      order.status === "PENDING" || order.status === "IN_PROGRESS" || order.status === "APPROVED"
                                              ),
                                              room
                                          )}
                                      </div>
            ...
            <div className="flex flex-col gap-2">
                ...
                >
                    ...
                    <ChevronUpIcon className={`w-6 h-6 transform transition-transform ${showAllLeaseOrders ? "rotate-180" : ""}`} />
                </div>
                <AnimatePresence>
                    {showAllLeaseOrders && (
                        <motion.div
