...
export default function LeaseOrderPanel(data) {
    ...
    const [leaserOrders, setLeaserOrders] = useState(
        data.data?.category === "HELLO_STUDIO" || data.data?.category === "HELLO_LANDLORD" ? data.data?.leaseOrdersProperty : null
    );
    ...
    const [showCurrentLeaseOrder, setShowCurrentLeaseOrder] = useState(leaserOrders?.length > 0 ? true : false);
    ...
    useEffect(() => {
        if (!client) {
            const fetchClient = async () => {
                try {
                    if (leaserOrders && leaserOrders.length > 0) {
                        const client = await axios.get(`/api/user?id=${leaserOrders[0]?.clientId}`);
                        if (client) {
            ...
            const fetchRooms = () => {
                const rooms = property.rooms.filter((room) => room.leaseOrdersRoom.length > 0);
                setRooms(rooms);
            };
    ...
    const aproveLeaseOrder = async (leaseOrder, contract) => {
        try {
            const dataRequest = {
                ...
                action: "APPROVED",
                propertyId: property.category === "HELLO_STUDIO" || property.category === "HELLO_LANDLORD" ? property.id : null,
                type: property.category === "HELLO_STUDIO" || property.category === "HELLO_LANDLORD" ? "PROPERTY" : "ROOM",
                roomId: property.category === "HELLO_STUDIO" || property.category === "HELLO_LANDLORD" ? null : leaseOrder.roomId,
            };
                        ...
                        >
                            ...
                            {(property?.category === "HELLO_ROOM" || property?.category === "HELLO_COLIVING") &&
                                ...
                                rooms.map((room) => {
                                    ...
                                    return (
                                        <div key={room.id} className="p-6 bg-[#ECF0F3] rounded-lg shadow-md border border-gray-200 space-y-4 my-4">
                                            {hasInProgressOrders ? (
                                                inProgressOrders.map((leaseOrder) => (
                                                    <div key={leaseOrder.id}>
                                                        <h2 className="text-2xl font-bold text-[#222B45] mb-2">{room.name}</h2>
                                                                        ...
                                                                        toast.custom((t) => (
                                                                            <div className="bg-white p-4 rounded shadow-md text-center">
                                                                                <p className="text-gray-800 mb-4">
                                                                                    ¿Estás seguro de que deseas aprobar esta orden de arrendamiento?
                                                                                </p>
                                                                ...
                                                                <button
                                                                    onClick={() => {
                                                                        toast.custom((t) => (
                                                                            <div className="bg-white p-4 rounded shadow-md text-center">
                                                                                <p className="text-gray-800 mb-4">
                                                                                    ¿Estás seguro de que deseas rechazar esta orden de arrendamiento?
                                                                                </p>
                                                                                <div className="flex justify-center gap-4">
                                                                                    <button
                                                                                        onClick={() => {
                                                ...
                                                <div className="flex flex-col justify-center items-center h-64 bg-white rounded-lg shadow-md p-6">
                                                    <h2 className="text-2xl font-bold text-[#222B45] mb-2">{room.name}</h2>
                                                    <p className="text-[#464E5F] text-lg font-semibold">No hay órdenes pendientes</p>
                                                </div>
                                            )}
                                        </div>
                                    );
                            ...
                            {(property?.category === "HELLO_STUDIO" || property?.category === "HELLO_LANDLORD") && leaserOrders?.length > 0 ? (
                                leaserOrders
                        ...
                        >
                            ...
                            {(property?.category === "HELLO_ROOM" || property?.category === "HELLO_COLIVING") &&
                                (rooms ? (
                                            ...
                                            >
                                                {hasFilteredOrders ? (
                                                    filteredOrders.map((leaseOrder) => (
                                                        <div key={leaseOrder.id}>
                                                            <h2 className="text-2xl font-bold text-[#222B45] mb-2">{room.name}</h2>
                                                            <LeaseOrderSection data={leaseOrder} formatDate={formatDate} isAll={true} />
                                                        </div>
                                                    ))
                                                ) : (
                                                    <div className="flex flex-col justify-center items-center h-64 bg-white rounded-lg shadow-md p-6">
                                    ...
                                    <div className="flex justify-center items-center h-64 bg-[#ECF0F3] rounded-lg shadow-md my-4">
                                        <p className="text-[#464E5F] text-lg font-semibold">No hay habitaciones con órdenes pendientes</p>
                                    </div>
                                ))}

                            {/* Propiedades sin room con precio */}
                            {(property?.category === "HELLO_STUDIO" || property?.category === "HELLO_LANDLORD") &&
                                (leaserOrders ? (
                                    leaserOrders
                                        // .filter((leaserOrder) => {
                                        //   return (
                                        ...
                                        // })
                                        .map((leaserOrder) => (
                                            <LeaseOrderSection data={leaserOrder} formatDate={formatDate} key={leaserOrder.id} isAll={true} />
                                        ))
                                ) : (
                                    <div className="flex justify-center items-center h-64 bg-[#ECF0F3] rounded-lg shadow-md my-4">
                                        <p className="text-[#464E5F] text-lg font-semibold">No hay órdenes pendientes</p>
