...
export default function UpdateProperty({ data = false, category, handleBack }) {
    ...
    const [predefineRental, setPredefineRental] = useState([]);
    ...
    useEffect(() => {
        const fetchOwners = async () => {
            ...
            const ownerEmail = res.data.find((owner) => owner.id == property?.ownerId);
        ...
        if (property) {
            ...
            setRentalPeriods(property?.rentalItems || []);
            setTypologyAndZone({
    ...
    useEffect(() => {
        const fetchRentalPeriods = async () => {
            ...
            setPredefineRental(res.data.rentalPeriods);
        };
        fetchRentalPeriods();
    }, []);
        ...
        try {
            ...
            try {
                ...
                if (newRooms.length > 0) {
                    const createdRooms = await axios.post("/api/admin/room", roomsFormated);
            ...
            let updateDataProperty = {
                ...
                amountOwner: parseFloat(price.price) - parseFloat(price.amountHelloflatmate) || 0,
                ...
                newRentalPeriods: newRentalPeriods.newRentalPeriods,
    ...
    const cloneProperty = async () => {
        try {
            ...
            const newCategory = response.data.property.category.name;
            ...
            <div className="w-full flex justify-center items-center">
                <div className="flex flex-col w-full md:hidden gap-2">
                    ...
                    <main className={`  flex flex-col gap-[2.5rem] grow m-4 text-[#0D171C]`}>
                        ...
                        <TypolyAndZoneSection data={typologyAndZone} setData={setTypologyAndZone} />
                        <div className="flex flex-col gap-2">
                            <h2 className="font-bold text-[1.2rem]">Propietario</h2>
                            <SearchEmail owners={owners} onSelect={handleEmailSelect} email={selectedEmail || property.owner?.email || ""} />{" "}
                        </div>
                        ...
                        {category !== "HELLO_ROOM" && category !== "HELLO_COLIVING" && category !== "HELLO_LANDLORD" && (
                            <CalendarSection data={calendarType} setData={setCalendarType} category={category} />
                        )}
                        {category !== "HELLO_ROOM" && category !== "HELLO_COLIVING" && category !== "HELLO_LANDLORD" && (
                            <RentalPeriodRoomEdit
                                ...
                                oldRentalPeriods={rentalPeriods.map((period) => {
                                    return { itemPeriodId: period.id, ...period.rentalPeriod };
                                })}
                            />
                        )}
                        <DescriptionSectionTemplate data={description || property.description} action={handleShowDescriptionModal} />
                        <RoomSectionTemplate
                        ...
                        <AmenitiesSection data={amenities || property.amenities} edit={<EditButton action={handleShowAmenitiesModal} />} />
                ...
                <AnimatePresence mode="wait">
                    {showSliderModal && (
                        <SliderModal initialImages={sliderImage} setNewImages={handleSliderImage} showModal={handleShowSliderModal} />
                    )}
                </AnimatePresence>
                <AnimatePresence mode="wait">
                    {showAddressModal && (
                        <AddressModal data={address} setData={handleAddressInfo} showModal={handleShowAddressModal} category={category} />
                    )}
                </AnimatePresence>
                <AnimatePresence mode="wait">
                    {showAmenitiesModal && <AmenitiesModalEdit data={amenities} setData={handleAmenitiesInfo} showModal={handleShowAmenitiesModal} />}
                </AnimatePresence>
                <AnimatePresence mode="wait">
                    {showAddRoom && (
                        <RoomAddModal
