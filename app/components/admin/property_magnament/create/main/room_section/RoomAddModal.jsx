...
export default function RoomAddModal({ data, setData, showModal, propertyId, category, rentalPeriods }) {
    ...
    const handleSubmit = async () => {
        ...
        if (!dataRoom?.numberBeds) return toast.error("Por favor, especifique una cantidad de camas");
        if (images.length === 0) return toast.error("Por favor, agrega al menos una imagen.");
        ...
        if (category === "HELLO_ROOM" || category === "HELLO_COLIVING" || category === "HELLO_LANDLORD" || category === "HELLO_STUDIO") {
            roomData = {
                ...dataRoom,
                temporaryId: uuidv4(),
                images: uploadedUrls,
    ...
    const handlePeriodChange = (index, field, value) => {
        const updatedPeriods = rentalPeriods.map((period, i) => (i === index ? { ...period, [field]: value } : period));
    ...
    const handleDescriptionChange = (index, value) => {
        const updatedDescription = description.map((desc, i) => (i === index ? { ...desc, text: value } : desc));
        setDescription(updatedDescription);
    };
              ...
              >
                ...
                <div className="w-full flex flex-col gap-3">
                    <div>
                        ...
                        <input
                            ...
                            onChange={(event) => setDataRoom({ ...dataRoom, price: event.target.value })}
                            className="number-input-no-appearance appearance-none outline-none w-full p-2 border border-gray-300 rounded"
                        />
                    </div>
                    <div>
                        ...
                        {description.map((description, index) => (
                            <li key={index} className="flex gap-3 items-center">
                                <input
                                    ...
                                    className="appearance-none outline-none w-full p-2 border border-gray-300 rounded"
                                />
                                <button type="button" onClick={() => handleRemoveDescription(index)} className="text-red-500">
                                    -
                                </button>
                            </li>
                        ))}
                    </ul>
                    <button type="button" onClick={handleAddDescription} className="mt-3 p-2 bg-blue-500 text-white rounded">
                        +
                    </button>
                </div>
                ...
                <div className="w-full flex gap-3 justify-center items-center flex-wrap">
                    ...
                    <div className="flex gap-2 px-3">
                        <input type="radio" name="bathroom" value="yes" checked={dataRoom.bathroom === "yes"} onChange={handleRadioChange} />
                        <label htmlFor="bathroom">Sí</label>
                    </div>
                    <div className="flex gap-2 px-3">
                        <input type="radio" name="bathroom" value="no" checked={dataRoom.bathroom === "no"} onChange={handleRadioChange} />
                ...
                <div className="w-full flex gap-3 justify-center items-center flex-wrap">
                    ...
                    <div className="flex gap-2 px-3">
                        <input type="radio" name="couple" value="yes" checked={dataRoom.couple === "yes"} onChange={handleRadioChange} />
                        <label htmlFor="couple">Sí</label>
                    </div>
                    <div className="flex gap-2 px-3">
                        <input type="radio" name="couple" value="no" checked={dataRoom.couple === "no"} onChange={handleRadioChange} />
