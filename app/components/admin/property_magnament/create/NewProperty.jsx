...
export default function NewProperty({ category, handleBack }) {
    ...
    // Modal handlers
    const handleShowDescriptionModal = () => setShowDescriptionModal(!showDescriptionModal);
    ...
    const submitRoom = async (data) => {
        ...
        if (data[0]?.amountOwner || data[0]?.amountHelloflatmate) {
            rooms = data.map((room) => ({
                ...
                amountOwner: parseInt(room?.price) - parseInt(room?.amountHelloflatmate) || 0,
    ...
    const createProperty = async () => {
        try {
            ...
            // Crear propiedad
            const propertyResponse = await axios.post("/api/admin/property", property);
    ...
    return (
        <div className="w-full flex justify-center items-center">
            ...
            <div className="flex flex-col w-full md:hidden gap-2 p-2">
                ...
                <main className={`  flex flex-col gap-[2.5rem] grow text-[#0D171C]`}>
                    <TitleSectionTemplate name={name} setName={setName} address={address} setAdress={setAddress} action={handleShowAddressModal} />
                    ...
                    <TypolyAndZoneSection data={typologyAndZone} setData={setTypologyAndZone} category={category} />
          ...
          {category !== "HELLO_ROOM" && category !== "HELLO_COLIVING" && category !== "HELLO_LANDLORD"  && (
            ...
            />
                    ...
                    <DescriptionSectionTemplate action={handleShowDescriptionModal} data={description} />
                    <RoomSectionTemplate
                        data={dataRoom}
                        setData={setRoomData}
                    ...
                    <MoreInfoSectionTemplate data={moreInfo} setData={setMoreInfo} action={handleShowMoreInfoModal} />
                    <SaveButton
                        action={() => {
            ...
            {showDescriptionModal && <DescriptionModal data={description} setData={setDescription} showModal={handleShowDescriptionModal} />}
            {showRoomEditModal && (
                <RoomAddModal
                    data={dataRoom}
                    setData={setRoomData}
            ...
            {showAddressModal && <AddressModal data={address} setData={setAddress} showModal={handleShowAddressModal} category={category} />}
        </div>
    );
}
