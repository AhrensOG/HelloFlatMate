...
import { CheckIcon, DocumentMagnifyingGlassIcon, MagnifyingGlassIcon, PencilIcon, PowerIcon, TrashIcon } from "@heroicons/react/24/outline";
...
export default function TableArticle({ data, role = "ADMIN" }) {
    ...
    const [showLeaseOrdersListModal, setShowLeaseOrdersListModal] = useState(false);
    const [showRentalPeriodModal, setShowRentalPeriodModal] = useState(false);

    const categories = ["HELLO_ROOM", "HELLO_STUDIO", "HELLO_COLIVING", "HELLO_LANDLORD"];
        ...
        const filtered = originalData
            .filter((item) => {
                ...
                const matchesSearch =
                    ...
                    `${item.city} ${item.street} ${item.streetNumber}`.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    return (
        <article className="flex flex-col justify-center items-center gap-4 w-full p-4">
                ...
                <div className="flex flex-col justify-center items-center gap-2 w-full">
                    <div className="flex justify-center items-center gap-2 w-full">
                        ...
                        {role === "ADMIN" && (
                            ...
                            <button onClick={toggleAlphabeticalOrder} className="p-2 bg-blue-500 text-white rounded">
                    ...
                    {role === "ADMIN" && (
                        <div className="w-full flex flex-wrap justify-center items-center gap-2">
                            ...
                            <button
                                onClick={() => setShowLeaseOrdersListModal(!showLeaseOrdersListModal)}
                                className="border border-resolution-blue px-5 py-2 max-w-[14rem] text-center w-auto rounded-md bg-resolution-blue text-white font-medium"
                            >
                                Ordenes de alquiler
                            </button>
            ...
            <div className="flex flex-wrap gap-4 mb-4 w-full justify-start lg:justify-center">
                <h2 className="text-xl font-bold text-primary w-full">Categorías y estados</h2>
                ...
                {statuses.map((status) => (
                    <label key={status} className="flex items-center gap-2">
                        ...
                        {status === "FREE" ? "Disponibles" : status === "RESERVED" ? "Pendientes" : "Reservados"}
                    </label>
                ))}
            </div>
            {/* Table */}
            <div className="w-full max-w-full border-2 border-primary rounded-lg overflow-x-auto overflow-y-auto max-h-[35rem]">
                <table className="relative min-w-full bg-white rounded-lg shadow-lg">
                    <thead className="bg-[#0e1863ff] text-white sticky top-0 z-10">
                        <tr>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Código</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Nombre</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Categoría</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Estado</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Dirección</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Código Postal</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Zona</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Tipología</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Área (m²)</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Habitaciones</th>
                            <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Baños</th>
                            {role === "ADMIN" && (
                                <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Propietario</th>
                            )}
                            {role === "ADMIN" && <th className="text-sm text-center px-1.5 py-2 font-medium border-b border-gray-200">Acciones</th>}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                        {filteredData.length > 0 ? (
                            filteredData.map((item, index) => (
                                ...
                                >
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.serial}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.name}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.category.replace(/_/g, "").toLowerCase()}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.status}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{`${item.street} ${item.streetNumber}, ${item.city}`}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.postalCode}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.zone}</td>
                                    ...
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.size}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.rooms?.length || 0}</td>
                                    <td className="px-1.5 py-2 text-center text-xs border-r">{item.bathrooms}</td>
                                    {role === "ADMIN" && <td className="px-1.5 py-2 text-center text-xs border-r">{item.owner?.email}</td>}
                                    {role === "ADMIN" && (
                                            ...
                                            <div className="flex gap-4 items-center justify-center">
                                                ...
                                                <div className="relative group inline-block">
                                                    <button
                                                        onClick={() =>
                                                            toast.promise(
                                                                item.isActive ? handleDesactivateProperty(item.id) : handleActiveProperty(item.id),
                                                                {
                                                                    loading: item.isActive ? "Desactivando..." : "Activando...",
                                                                    success: item.isActive ? "Desactivado correctamente" : "Activado correctamente",
                                                                    error: item.isActive ? "Error al desactivar" : "Error al activar",
                                                                }
                                                            )
                                                        }
                                                        className={`${
                                                ...
                                                <div className="relative group inline-block">
                                                    <button
                                                        onClick={() => router.push(`/pages/admin/update/${item.id}/${item.category}`)}
                                                ...
                                                <div className="relative group inline-block">
                                                    <button
                                                        onClick={() => router.push(`/pages/admin/lease_order/${item.id}`)}
                            ...
                            <tr>
                                <td className="px-4 py-4 text-center text-gray-500" colSpan="13">
                                    No hay resultados
                                </td>
                            </tr>
                        )}
            ...
            {showLeaseOrdersListModal && <LeaseOrdersListModal onClose={() => setShowLeaseOrdersListModal(false)} />}
            {showLeaseOrderModal && <CreateLeaseOrderModal data={data} onClose={() => setShowLeaseOrderModal(false)} />}
            {showRentalPeriodModal && <AddRentalPeriodsModal onClose={() => setShowRentalPeriodModal(false)} />}
        </article>
    );
}
