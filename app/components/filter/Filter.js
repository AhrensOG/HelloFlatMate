    ...
    return (
        <AnimatePresence mode="wait">
            {true && <motion.aside
                ...
                className={`${plus_jakarta.className} flex flex-col gap-3 w-full h-[70vh] overflow-hidden mt-2 self-end relative`}
                style={{ maxHeight: "70vh" }}
            >
                <div className="h-[5vh] w-full flex items-center px-4">
                    <span>
                        <Image src={"/btn-close.svg"} alt="Close button" layout="responsive" width={24} height={24} />
                    </span>
                    <h2 className="text-center grow text-lg font-bold">Filter</h2>
                </div>
                <div className="overflow-auto">
                    <section className="flex flex-col gap-3 px-4">
                        <h3 className="text-[1.37rem] font-bold text-[#1C1C21]">Ubicacion</h3>
                        <div className="flex justify-between items-center h-[5vh]">
                            <p className="text-end font-normal">Direccion 1, avenida</p>
                            <span className="h-full">
                                <Image src={"/filter/location-icon.svg"} alt="Location icon" width={28} height={28} />
                            </span>
                        </div>
                    </section>
