...
const CountrySelect = ({ value, onChange, containerCustomClass = "", spanCustomClass = "" }) => {
    ...
    const filteredCountries = countryList.filter((country) => country.name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const t = useTranslations("user_profile_v2.inf_per");
    return (
        <div className="relative w-full">
            {/* Campo de selección */}
            <div
                onClick={() => setIsOpen(!isOpen)}
                className={`${containerCustomClass} border rounded-md px-3 py-2 bg-white ${
                    isOpen && "ring-1 ring-blue-300"
                } outline-none cursor-pointer flex justify-between items-center`}
            >
                <span className={` ${spanCustomClass} text-gray-700 text-base`}>{selectedValue || t("select_country")}</span>
            </div>
