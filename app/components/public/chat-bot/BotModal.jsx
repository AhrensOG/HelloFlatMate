...
const faq = {
  ...
  "¿Hay pisos solo para chicas?":
    "helloflatmate dispone de pisos mixtos y pisos solo para chicas.",
...
export default function BotModal({ onClose }) {
  ...
  const sendMessage = (msg) => {
    ...
    if (chat.length === 0) {
      setChat((prevChat) => [
        ...
        {
          role: "bot",
          content: "Bienvenido/a! Estas son algunas preguntas frecuentes:",
        },
      ]);
      setShowMenu(true);
    } else {
  ...
  const handleBotResponse = (question) => {
    if (faq[question]) {
      ...
      setChat((prevChat) => [
        ...
        {
          role: "bot",
          content: "Lo siento, estoy entrenando para poder responderte muy pronto.",
        },
      ]);
    }
  };
  ...
  const finishChat = () => {
    setChat([
      ...chat,
      { role: "bot", content: "¡Gracias por preguntar! Hasta luego." },
    ]);
    setShowMenu(false);
    setIsResponseShown(false);
  };
  ...
  return (
    ...
    >
      <div className="w-full bg-blue-600 rounded-tl-lg rounded-tr-lg flex justify-between items-center px-3 py-3 gap-2">
        <div className="flex gap-2">
          <Image
            src={"/chat_bot/bot3.png"}
            width={50}
            height={50}
            alt="helloflatmate - logo"
          />
          <div className="flex flex-col justify-center items-start">
            <h2 className="text-white font-semibold">helloflatmate</h2>
            <span className="text-white font-light text-xs">hello bot</span>
          </div>
        </div>
      ...
      <div className="w-full flex-grow bg-white p-4 flex flex-col overflow-y-auto space-y-4">
        ...
        {isResponseShown && (
          <div className="mt-4 flex justify-between">
            <button
              onClick={comeBackMenu}
              className="bg-blue-200 hover:bg-blue-100 transition duration-300 p-1 px-2 rounded-2xl text-blue-600 text-sm font-medium"
            >
              Volver al menú principal
            </button>
            <button
              onClick={finishChat}
              className="bg-teal-200 hover:bg-teal-100 transition duration-300 p-1 px-2 rounded-2xl text-teal-600 text-sm font-medium"
            >
              Finalizar
            </button>
          </div>
